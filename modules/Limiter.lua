		...
		if input then
			...
			if rules then
				...
				if threshold and limit then
					...
					-- @todo
					-- interpolate between the threshold and limits?
					--[[
					local overage
					if value <= lower_threshold then
						-- zoomed out
						overage = lower_threshold - value
						local range = lower_threshold - lower_limit
						...
						local c = range
						local d = range
						local interpolated = easing.outQuart(t, b, c, d)

					elseif value >= upper_threshold then
						-- zoomed in
						overage = value - upper_threshold
						local range = upper_limit - upper_threshold

					end
					]]--

					-- hard cap the value to the limits for safety
